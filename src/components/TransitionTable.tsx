...
export const TransitionTable = memo(({ automaton, title }: Props) => {
  ...
  return (
    <div className={`rounded-2xl flex flex-col border backdrop-blur-xl transition-all duration-300 overflow-hidden ${
      ...
      <div className={`px-4 sm:px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b shrink-0 ${
        isDark ? 'border-white/5 bg-black/40' : 'border-slate-200 bg-slate-50/50'
      }`}>
        <h3 className={`font-bold tracking-wide uppercase text-xs sm:text-sm flex items-center gap-2 ${
        ...
        <div className="relative w-full sm:w-auto">
          ...
          <input
            ...
            className={`w-full sm:w-48 pl-9 pr-3 py-2 sm:py-1.5 text-xs rounded-lg outline-none transition-all border ${
              isDark 
      ...
      <div className="w-full overflow-x-auto custom-scrollbar relative max-h-[350px] sm:max-h-[400px]">
        <table className="w-full text-left border-collapse whitespace-nowrap">
          <thead className={`sticky top-0 z-10 shadow-sm backdrop-blur-xl ${
            isDark ? 'bg-[#080810]/95' : 'bg-slate-50/95'
          }`}>
            <tr>
              <th className={`px-4 py-3 text-[11px] sm:text-xs font-black tracking-widest uppercase border-b ${
              ...
              {automaton.alphabet.map((symbol) => (
                <th 
                  ...
                  className={`px-4 py-3 text-[11px] sm:text-xs font-black tracking-widest uppercase border-b transition-colors cursor-default ${
                    isDark ? 'border-white/10 text-slate-400' : 'border-slate-200 text-slate-500'
            ...
            {filteredStates.length > 0 ? (
              filteredStates.map((state) => {
                ...
                return (
                  ...
                  >
                    <td className={`px-4 py-3 flex items-center gap-2 transition-colors ${
                    ...
                    {isNFA && (
                      <td className={`px-4 py-3 text-xs transition-colors ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                    ...
                    {automaton.alphabet.map((symbol) => {
                      ...
                      return (
                        <td key={symbol} className={`px-4 py-3 transition-colors ${cellBg} ${
      ...
      <div className={`px-4 sm:px-5 py-3 text-[9px] sm:text-[10px] uppercase tracking-widest font-bold flex flex-wrap gap-4 sm:gap-5 border-t shrink-0 ${
