...
export const EducationPanel = () => {
  ...
  const steps = [
    {
      id: 1, title: t.step1Title, icon: <Search className="w-5 h-5" />,
      content: (
        <div className="space-y-3 text-sm">
          <p className={isDark ? 'text-slate-400' : 'text-slate-600'}>{t.step1Desc}</p>
          <div className={`p-4 rounded-xl border font-mono transition-colors ${isDark ? 'bg-black/40 border-white/5 text-slate-300' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
            <div className="flex items-center gap-2"><span className="text-violet-500 font-bold">Input :</span> {regex}</div>
            <div className="mt-2 flex items-center gap-2"><span className="text-pink-500 font-bold">Postfix:</span> {postfix}</div>
          </div>
        </div>
      )
    },
    {
      id: 2, title: t.step2Title, icon: <GitCommit className="w-5 h-5" />,
      content: (
        <div className="space-y-3 text-sm">
    ...
    {
      id: 3, title: t.step3Title, icon: <GitBranch className="w-5 h-5" />,
      content: (
        <div className="space-y-3 text-sm">
          ...
          <div className={`max-h-[180px] overflow-auto custom-scrollbar border rounded-xl ${isDark ? 'bg-black/20 border-white/5' : 'bg-white border-slate-200 shadow-sm'}`}>
            <table className="w-full text-xs text-left border-collapse">
              <thead className={`sticky top-0 z-10 backdrop-blur-md ${isDark ? 'bg-[#080810]/90 text-slate-400 border-white/5' : 'bg-slate-50 text-slate-500 border-slate-200'}`}>
                <tr><th className="px-4 py-2 border-b uppercase tracking-widest font-black text-[10px]">{t.step3TableCol1}</th><th className="px-4 py-2 border-b uppercase tracking-widest font-black text-[10px]">{t.step3TableCol2}</th></tr>
              </thead>
              <tbody className={`divide-y font-mono ${isDark ? 'divide-white/5' : 'divide-slate-100'}`}>
                {Object.entries(epsClosures).map(([s, cls]) => (
                  ...
                  >
                    <td className="px-4 py-2 font-bold text-violet-400">{s}</td>
                    <td className="px-4 py-2 text-sky-400">{`{${cls.join(', ')}}`}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-[10px] italic text-slate-500 text-center">{t.step3Hint}</p>
        </div>
      )
    },
    {
      id: 4, title: t.step4Title, icon: <BookOpen className="w-5 h-5" />,
      content: (
        <div className="space-y-3 text-sm">
          ...
          <div className={`max-h-[180px] overflow-auto custom-scrollbar border rounded-xl ${isDark ? 'bg-black/20 border-white/5' : 'bg-white border-slate-200 shadow-sm'}`}>
            <table className="w-full text-xs text-left border-collapse">
              <thead className={`sticky top-0 z-10 backdrop-blur-md ${isDark ? 'bg-[#080810]/90 text-slate-400 border-white/5' : 'bg-slate-50 text-slate-500 border-slate-200'}`}>
                <tr><th className="px-4 py-2 border-b uppercase tracking-widest font-black text-[10px]">{t.step4TableCol1}</th><th className="px-4 py-2 border-b uppercase tracking-widest font-black text-[10px]">{t.step4TableCol2}</th></tr>
              </thead>
              <tbody className={`divide-y font-mono ${isDark ? 'divide-white/5' : 'divide-slate-100'}`}>
                {dfa.mapping && Object.entries(dfa.mapping).map(([dfaS, nfaSet]) => (
                  ...
                  >
                    <td className="px-4 py-2 font-bold text-pink-400">{dfaS}</td>
                    <td className="px-4 py-2 text-violet-400">{`{${nfaSet.join(', ')}}`}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-[10px] italic text-slate-500 text-center">{t.step4Hint}</p>
        </div>
      )
    },
    {
      id: 5, title: t.step5Title, icon: <BarChart className="w-5 h-5" />,
      content: (
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-2 gap-4">
            <div 
              className={`p-4 rounded-xl border transition-all cursor-pointer ${isDark ? 'bg-violet-500/5 border-violet-500/20 hover:bg-violet-500/10' : 'bg-violet-50 border-violet-200 hover:bg-violet-100'}`}
              onMouseEnter={() => setTestActiveElements(nfa.states, [])}
              onMouseLeave={() => clearTestActiveElements()}
            >
              <h4 className={`font-black uppercase tracking-widest text-[10px] mb-2 ${isDark ? 'text-violet-400' : 'text-violet-600'}`}>{t.step5NfaStruct}</h4>
            ...
            <div 
              className={`p-4 rounded-xl border transition-all cursor-pointer ${isDark ? 'bg-sky-500/5 border-sky-500/20 hover:bg-sky-500/10' : 'bg-sky-50 border-sky-200 hover:bg-sky-100'}`}
              onMouseEnter={() => setTestActiveElements(dfa.states, [])}
              onMouseLeave={() => clearTestActiveElements()}
            >
              <h4 className={`font-black uppercase tracking-widest text-[10px] mb-2 ${isDark ? 'text-sky-400' : 'text-sky-600'}`}>{t.step5DfaStruct}</h4>
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl border backdrop-blur-xl mb-8 overflow-hidden transition-all duration-300 ${
        isDark ? 'bg-white/2 border-white/5 shadow-xl' : 'bg-white/80 border-slate-200 shadow-lg'
      }`}
    >
      <div className={`px-6 py-4 flex items-center justify-between border-b ${isDark ? 'border-white/5 bg-black/20' : 'border-slate-100 bg-slate-50/50'}`}>
        <div className={`flex items-center gap-2 font-black uppercase tracking-widest text-xs ${isDark ? 'text-violet-400' : 'text-violet-700'}`}>
      ...
      <div className="p-6">
        ...
        <div className={`flex items-center justify-between mt-8 pt-5 border-t ${isDark ? 'border-white/5' : 'border-slate-100'}`}>
          <button
            onClick={() => setStep(Math.max(1, step - 1))}
            disabled={step === 1}
            className={`px-4 py-2.5 text-xs font-bold uppercase tracking-widest rounded-xl transition-all flex items-center gap-2 disabled:opacity-30 border ${
          ...
          <button
            ...
            disabled={step === 5}
            className="px-5 py-2.5 text-xs font-bold uppercase tracking-widest rounded-xl bg-violet-600 hover:bg-violet-500 text-white transition-all active:scale-95 disabled:opacity-30 shadow-lg shadow-violet-500/20 flex items-center gap-2"
          >
